...
def dependencies_in(asm_file_paths, source_dir="")
    ...
    asm_file_paths.each do |asm_file_path|
        if !dependencies.has_key? asm_file_path
            asm_dependencies, bin_dependencies =
                shallow_dependencies_of asm_file_path, source_dir
...
def shallow_dependencies_of(asm_file_path, source_dir="")
    ...
    File.each_line asm_file_path do |line|
        ...
        path = File.join(source_dir, path) if !source_dir.empty?
        if keyword == "INCLUDE"
...
dependencies_in(ARGV, source_dir).each do |file, dependencies|
